              ...
              children: [
                Text(
                  ...
                  style: TextStyle(
                    fontSize: 16,
                    fontWeight: FontWeight.normal,
                    color: widget.cijenaTrenutna > widget.item.cijena / widget.item.kolicina
                        ? Colors.green
                        : Colors.red,
                  ),
                ),
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          actions: <Widget>[
            ...
            TextButton(
              onPressed: () async{
                int selectedQuantity = int.tryParse(_quantityController.text) ?? 0;

                if(selectedQuantity<widget.item.kolicina){
                    var kolicina1 = widget.item.kolicina - selectedQuantity;
                    var cijena1 = widget.item.cijena - (widget.cijenaTrenutna*selectedQuantity);
                        final Dionica dio = Dionica(id:widget.item.id,cijena:cijena1,kolicina: kolicina1,simbol:widget.item.simbol,ime:widget.item.ime,slika:widget.item.slika,dionicaId:widget.item.dionicaId);
                          await DatabaseHelper.updateDionica(dio);
                      double kes=money!+(widget.cijenaTrenutna * selectedQuantity);
                      money = kes;
                      updateMoney(money!);
                      ...
                      MaterialPageRoute(builder: (context) => NavBar()),
                        ...
                        money = kes;
                        updateMoney(money!);
                        final Dionica dio = Dionica(id:widget.item.id,cijena:widget.item.cijena,kolicina: widget.item.kolicina,simbol:widget.item.simbol,ime:widget.item.ime,slika:widget.item.slika,dionicaId:widget.item.dionicaId);
